...
export class SignalsService {
  ...
  async buildSignalContext(options: {
    ...
    if (liveSnapshot?.orderbook) {
      ...
      if (live.bestBid > 0 && live.bestAsk > 0) {
        const liveBidWallUsd = Array.isArray(live.bidWalls)
          ? live.bidWalls.reduce(
              (sum: number, w: { price: number; volume: number }) =>
                sum + w.price * w.volume,
              0,
            )
          : undefined;
        const liveAskWallUsd = Array.isArray(live.askWalls)
          ? live.askWalls.reduce(
              (sum: number, w: { price: number; volume: number }) =>
                sum + w.price * w.volume,
              0,
            )
        ...
        orderBook = {
          ...
          mid: live.mid || (live.bestBid + live.bestAsk) / 2,
          spread: live.spread || live.bestAsk - live.bestBid,
          spreadPct:
            live.spreadPct ||
            ((live.bestAsk - live.bestBid) / live.bestBid) * 100,
          imbalance: live.imbalance,
          depth: Math.max(orderBook.depth ?? 0, live.depth ?? 0),
          ...(liveBidWallUsd !== undefined && liveBidWallUsd > 0
            ? { bidWallUsd: liveBidWallUsd }
            : {}),
          ...(liveAskWallUsd !== undefined && liveAskWallUsd > 0
            ? { askWallUsd: liveAskWallUsd }
            : {}),
        };
      }
    }
    const profileSummary = this.resolveHorizontalVolumeProfileSummary(
      liveSnapshot?.trades?.horizontalVolumeProfileSummary,
    );
    if (profileSummary) {
      orderFlow.horizontalVolumeProfile = profileSummary;
    }
    const liquidityMap = this.resolveLiquidityMapSummaryByScope(
      liveSnapshot?.trades?.liquidityMap,
    );
    if (liquidityMap) {
      orderFlow.liquidityMap = liquidityMap;
    }
    // Enrich trade flow with live data when QuestDB trades are stale
    if (liveSnapshot?.trades && liveSnapshot.trades.tradeCount > 0) {
      const lt = liveSnapshot.trades;
      if (
        !((orderFlow.tradeCount ?? 0) > 0) ||
        lt.tradeCount > (orderFlow.tradeCount ?? 0)
      ) {
        orderFlow.tradeCount = lt.tradeCount;
        orderFlow.buyVolume = lt.buyVolume;
        orderFlow.sellVolume = lt.sellVolume;
        orderFlow.aggressiveBuyVolume = lt.aggressiveBuyVolume;
        orderFlow.aggressiveSellVolume = lt.aggressiveSellVolume;
        orderFlow.avgTradeSize = lt.avgTradeSize;
        orderFlow.maxTradeSize = lt.maxTradeSize;
        const totalBuySell = lt.buyVolume + lt.sellVolume;
        orderFlow.deltaRatio =
          totalBuySell > 0 ? lt.buyVolume / totalBuySell : 0.5;
        orderFlow.delta = lt.buyVolume - lt.sellVolume;
        orderFlow.cvd = lt.buyVolume - lt.sellVolume;
        if (lt.vwap > 0) orderFlow.vwap = lt.vwap;
      }
    }
    const fallbackMid = Number(orderBook?.mid ?? NaN);
    if (!((orderFlow.tradeCount ?? 0) > 0)) {
      this.logger.warn(
        `[buildSignalContext] no trades in flow window for ${symbol} — trade flow metrics will be null/zero`,
      );
    }
    if (
      !(Number(orderFlow.vwap) > 0) &&
      Number.isFinite(fallbackMid) &&
      fallbackMid > 0
    ) {
    ...
    const totalVolume = Math.max(
      0,
      Number(orderFlow.buyVolume ?? 0) + Number(orderFlow.sellVolume ?? 0),
    );
    if (
      !(Number(orderFlow.avgTradeSize) > 0) &&
      Number(orderFlow.tradeCount ?? 0) > 0
    ) {
    ...
    const latestD1Time = Number(
      normalizedCandles.d1?.[normalizedCandles.d1.length - 1]?.time ?? 0,
    );
    const latestH4Time = Number(
      normalizedCandles.h4?.[normalizedCandles.h4.length - 1]?.time ?? 0,
    );
    const latestH1Time = Number(
      normalizedCandles.h1?.[normalizedCandles.h1.length - 1]?.time ?? 0,
    );
    ...
    return {
      ...
      orderBook,
      orderFlow,
      volumeProfile: profileSummary
        ? {
            summary: profileSummary,
            levels: liveSnapshot?.trades?.horizontalVolumeProfileLevels ?? [],
          }
        : undefined,
      contextFeatures,
    };
  ...
  private buildOrderBookSnapshot(symbol: string, rows: any[]) {
    const bids = rows.filter(
      (x: any) => String(x.side).toLowerCase() === 'bid',
    );
    const asks = rows.filter(
      (x: any) => String(x.side).toLowerCase() === 'ask',
    );

    let bestBid = bids.reduce(
      (max, x: any) => Math.max(max, Number(x.price ?? 0)),
      0,
    );
    let bestAsk = asks.reduce((min, x: any) => {
      const p = Number(x.price ?? 0);
      if (!p) return min;
      return min === 0 ? p : Math.min(min, p);
    }, 0);
    // Normalize crossed book: synthesize from mid if bid >= ask
    if (bestBid > 0 && bestAsk > 0 && bestBid >= bestAsk) {
      const rawMid = (bestBid + bestAsk) / 2;
      const synSpread = rawMid * 0.0001;
      bestBid = rawMid - synSpread / 2;
      bestAsk = rawMid + synSpread / 2;
    }
  ...
  private buildOrderFlowSnapshot(
    symbol: string,
    rows: any[],
  ): OrderFlowSnapshot {
  ...
  private normalizeCandles(rows: unknown): Array<{
    ...
    for (const row of rows) {
      ...
      const time = this.resolveTimestampMs(
        candle.time ?? candle.ts ?? candle.closeTime,
      );
      ...
      const highCandidate =
        Number.isFinite(highRaw) && highRaw > 0 ? highRaw : close;
      const lowCandidate =
        Number.isFinite(lowRaw) && lowRaw > 0 ? lowRaw : close;
  ...
  private getLiveSnapshotCandles(
    symbol: string,
    interval: TimeFrame,
  ): Array<{
    time: number;
    ...
    const key: 'h1' | 'h4' | 'd1' =
      interval === TimeFrame.h4
        ...
        ? 'd1'
        : 'h1';
    const raw = Array.isArray(snapshot.candles?.[key])
      ? snapshot.candles[key]
      : [];
  ...
  private async loadCandlesWithStoreFirst(
    ...
    if (merged.length < minRequired) {
      const fallbackCount = Math.max(
        this.resolveFallbackLastN(interval, days),
        minRequired * 2,
      );
  ...
  private daysToMs(days: number): number {
    const normalizedDays =
      Number.isFinite(days) && days > 0 ? Math.floor(days) : 1;
  ...
  private resolveHorizontalVolumeProfileSummary(
    value: unknown,
  ): HorizontalVolumeProfileSummary | undefined {
    if (!value || typeof value !== 'object') return undefined;
    const row = value as Record<string, unknown>;
    const bucketSize = Number(row.bucketSize ?? NaN);
    const updatedAt = Number(row.updatedAt ?? NaN);
    const totalVolume = Number(row.totalVolume ?? NaN);
    if (
      !(bucketSize > 0) ||
      !(updatedAt > 0) ||
    ...
    return {
      ...
      bucketSize,
      bucketPrecision: Number(row.bucketPrecision ?? 0) || 0,
      windowStart: Number(row.windowStart ?? 0) || 0,
      windowEnd: Number(row.windowEnd ?? updatedAt) || updatedAt,
      updatedAt,
      tradeCount: Number(row.tradeCount ?? 0) || 0,
      buyVolume: Number(row.buyVolume ?? 0) || 0,
      sellVolume: Number(row.sellVolume ?? 0) || 0,
      totalVolume,
      deltaVolume: Number(row.deltaVolume ?? 0) || 0,
      pocPrice: this.resolveNullableNumber(row.pocPrice),
      valueAreaLow: this.resolveNullableNumber(row.valueAreaLow),
      valueAreaHigh: this.resolveNullableNumber(row.valueAreaHigh),
      nearestHighVolumeNodeAbove: this.resolveNullableNumber(
        row.nearestHighVolumeNodeAbove,
      ),
      nearestHighVolumeNodeBelow: this.resolveNullableNumber(
        row.nearestHighVolumeNodeBelow,
      ),
      nearestLowVolumeNodeAbove: this.resolveNullableNumber(
        row.nearestLowVolumeNodeAbove,
      ),
      nearestLowVolumeNodeBelow: this.resolveNullableNumber(
        row.nearestLowVolumeNodeBelow,
      ),
      profileSkew: Number(row.profileSkew ?? 0) || 0,
      concentrationScore: Number(row.concentrationScore ?? 0) || 0,
      localImbalance: Number(row.localImbalance ?? 0) || 0,
      buySellDeltaAtCurrentRegion:
        Number(row.buySellDeltaAtCurrentRegion ?? 0) || 0,
      distanceToPocBps: this.resolveNullableNumber(row.distanceToPocBps),
    };
  }

  private resolveNullableNumber(value: unknown): number | null {
    const n = Number(value);
    return Number.isFinite(n) ? n : null;
  }

  private resolveLiquidityMapSummaryByScope(
    value: unknown,
  ): LiquidityMapSummaryByScope | undefined {
    if (!value || typeof value !== 'object') return undefined;
    const row = value as Record<string, unknown>;
    const scopes = [
      'rolling_30m',
    ...
    const out: LiquidityMapSummaryByScope = {};
    for (const scope of scopes) {
      const scopeRow = row[scope];
      if (!scopeRow || typeof scopeRow !== 'object') continue;
      const src = scopeRow as Record<string, unknown>;
      const updatedAt = Number(src.updatedAt ?? NaN);
      if (!(updatedAt > 0)) continue;
      out[scope] = {
        scope,
        updatedAt,
        bucketSize: Number(src.bucketSize ?? 0) || 0,
        pocPrice: this.resolveNullableNumber(src.pocPrice),
        valueAreaLow: this.resolveNullableNumber(src.valueAreaLow),
        valueAreaHigh: this.resolveNullableNumber(src.valueAreaHigh),
        distanceToPocBps: this.resolveNullableNumber(src.distanceToPocBps),
        distanceToValueAreaLowBps: this.resolveNullableNumber(
          src.distanceToValueAreaLowBps,
        ),
        distanceToValueAreaHighBps: this.resolveNullableNumber(
          src.distanceToValueAreaHighBps,
        ),
        nearestHvnAboveDistanceBps: this.resolveNullableNumber(
          src.nearestHvnAboveDistanceBps,
        ),
        nearestHvnBelowDistanceBps: this.resolveNullableNumber(
          src.nearestHvnBelowDistanceBps,
        ),
        nearestLvnAboveDistanceBps: this.resolveNullableNumber(
          src.nearestLvnAboveDistanceBps,
        ),
        nearestLvnBelowDistanceBps: this.resolveNullableNumber(
          src.nearestLvnBelowDistanceBps,
        ),
        nearestLiquidityWallAboveDistanceBps: this.resolveNullableNumber(
          src.nearestLiquidityWallAboveDistanceBps,
        ),
        nearestLiquidityWallBelowDistanceBps: this.resolveNullableNumber(
          src.nearestLiquidityWallBelowDistanceBps,
        ),
        strongestClusterAboveDistanceBps: this.resolveNullableNumber(
          src.strongestClusterAboveDistanceBps,
        ),
        strongestClusterBelowDistanceBps: this.resolveNullableNumber(
          src.strongestClusterBelowDistanceBps,
        ),
        localOrderbookImbalance: Number(src.localOrderbookImbalance ?? 0) || 0,
        localBidLiquidityDensity:
          Number(src.localBidLiquidityDensity ?? 0) || 0,
        localAskLiquidityDensity:
          Number(src.localAskLiquidityDensity ?? 0) || 0,
        thinZoneAboveDistanceBps: this.resolveNullableNumber(
          src.thinZoneAboveDistanceBps,
        ),
        thinZoneBelowDistanceBps: this.resolveNullableNumber(
          src.thinZoneBelowDistanceBps,
        ),
        liquidityPullVelocity: Number(src.liquidityPullVelocity ?? 0) || 0,
        liquidityAddVelocity: Number(src.liquidityAddVelocity ?? 0) || 0,
        liquidityConsumeVelocity:
          Number(src.liquidityConsumeVelocity ?? 0) || 0,
        liquidityInstabilityScore:
          Number(src.liquidityInstabilityScore ?? 0) || 0,
        localAbsorptionScore: Number(src.localAbsorptionScore ?? 0) || 0,
        localAggressionImbalance:
          Number(src.localAggressionImbalance ?? 0) || 0,
        liquidityClusterConfluenceScore:
          Number(src.liquidityClusterConfluenceScore ?? 0) || 0,
        liquidityGapPressure: Number(src.liquidityGapPressure ?? 0) || 0,
        profileOrderbookAlignmentScore:
          Number(src.profileOrderbookAlignmentScore ?? 0) || 0,
        currentPriceInValueArea: Number(src.currentPriceInValueArea ?? 0) || 0,
      };
    }
