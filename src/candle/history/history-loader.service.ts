...
export class HistoryLoaderService {
    ...
    async getHistory(options: History & { logMeta?: WarmupLogMeta }): Promise<Candle[]> {
        const { connectorType, marketType, symbols, interval, days, gapDays, logMeta } = options;
        ...
        for (const s of symbols) {
            ...
            this.warmupLog(
                `${logPrefix} getHistory from=${new Date(from).toISOString()} to=${new Date(end).toISOString()}`,
            );
