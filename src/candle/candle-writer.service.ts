...
export class CandleWriterService {
  ...
  private dedupeHistoryCandles(
    ...
    for (const c of sorted) {
      // time в ms
      const key = `${symbol}|${String(interval)}|${connectorType}|${marketType}|${c.time}`;
      map.set(key, c);
    }
  ...
  async upsertFinalCandle(
    ...
    const tsMs = Math.trunc(candle.time);
    if (!Number.isFinite(tsMs) || tsMs < CandleWriterService.MIN_REASONABLE_TS_MS) return;
    ...
    const batch: ILPWriteOptions[] = list
      .filter((c) => {
        ...
        if (
          ...
          this.logger.warn(
            `[candles] writeBatch skipped invalid timestamp | symbol=${c.symbol} interval=${String(
              c.interval,
            )} connectorType=${c.connectorType} marketType=${c.marketType} ts=${c.ts}`,
          );
          return false;
        }
  ...
  private async tryResume(logPrefix: string): Promise<boolean> {
    ...
    this.logger.warn(`${logPrefix} [WAL] ALTER TABLE ${this.CHANNEL} RESUME WAL`);
    return true;
  }
  ...
  private async waitUntilWalUnsuspended(logMeta: WarmupLogMeta): Promise<boolean> {
    const logPrefix = this.warmupPrefix(logMeta);
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      const stateBefore = await this.questDbQueryService.getWalState(this.CHANNEL);
      ...
      const stateAfter = await this.questDbQueryService.getWalState(this.CHANNEL);
  ...
  async bulkInsertHistory(
    ...
    if (rejected.length > 0) {
      ...
      rejected.slice(0, 5).forEach(({ candle, reason }, idx) => {
        this.logger.warn(
          `${logPrefix} rejected[${idx}] ${reason} | time=${candle.time} ts=${new Date(
            candle.time,
          ).toISOString()} o=${candle.open} h=${candle.high} l=${candle.low} c=${candle.close} v=${candle.volume}`,
        );
      });
      if (rejected.length > 5) {
        this.logger.warn(`${logPrefix} ... and ${rejected.length - 5} more rejected.`);
      }
    }
