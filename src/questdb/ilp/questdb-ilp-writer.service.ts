...
export class QuestDbIlpWriterService implements OnModuleDestroy {
    ...
    private connect(): void {
        ...
        try {
            ...
            this.connected = false;
            this.logger.warn(
                `[ILP] connect threw: ${error instanceof Error ? error.message : String(error)}`,
            );
            this.scheduleReconnect();
        }
    }
    ...
    private scheduleReconnect(): void {
        ...
        const expBackoff = Math.min(
            this.reconnectMaxMs,
            this.reconnectBaseMs * Math.pow(2, Math.min(this.reconnectAttempt - 1, 6)),
        );
        const jitter = Math.floor(Math.random() * Math.max(100, Math.floor(expBackoff * 0.2)));
