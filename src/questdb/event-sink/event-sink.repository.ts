...
export class EventSinkRepository implements OnModuleInit, OnModuleDestroy {
  ...
  getRuntimeStats(windowMinutes: number): {
    ...
    const safeWindow = Math.max(
      1,
      Math.min(this.runtimeWindowMaxMinutes, Math.floor(Number(windowMinutes) || 1)),
    );
    const nowMinute = Math.floor(Date.now() / 60_000);
    this.pruneRuntimeBuckets(nowMinute);
    return {
      eventsLast1m: this.countRuntimeEventsForLastMinutes(1, nowMinute),
      eventsLastWindow: this.countRuntimeEventsForLastMinutes(safeWindow, nowMinute),
  ...
  async getPaginated(options: PaginateOptions) {
    let { page, limit, category, symbol, from, to, search, includePayload } = options;
    ...
    if (from)
      where.push(`ts >= to_timestamp(${Number(from)}::long)`);
    if (to)
      where.push(`ts <= to_timestamp(${Number(to)}::long)`);

    if (search) where.push(`payload ~ '${this.escape(search)}'`);

    const whereSQL = where.length ? `WHERE ${where.join(' AND ')}` : '';
    const offset = (page - 1) * limit;
    const limitClause = offset > 0 ? `LIMIT ${offset}, ${limit}` : `LIMIT ${limit}`;
    ...
    const selectWithoutPayload = `
        SELECT ts,
               ...
               marketType
      `;
    const selectClause = wantsPayload ? selectWithPayload : selectWithoutPayload;
    ...
    try {
      ...
      const isBufferOverflow = message.includes('not enough space in send buffer');
    ...
    const hasPayload = wantsPayload && data.some(row => row?.payload !== undefined);
    if (!hasPayload) {
  ...
  private recordRuntimeEventTs(ts?: number): void {
    ...
    if (!Number.isFinite(tsMs) || tsMs < EventSinkRepository.MIN_REASONABLE_TS) {
      return;
    }
  ...
  private countRuntimeEventsForLastMinutes(minutes: number, nowMinute: number): number {
    let total = 0;
