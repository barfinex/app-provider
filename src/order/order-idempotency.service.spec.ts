...
async function createWithIdempotency<T>(options: {
    ...
    const replayAfterWait = await service.waitForFinalResponse<T>(idempotencyKey, {
...
describe('OrderIdempotencyService', () => {
    ...
    afterAll(() => {
        ...
        if (originalRedisPort !== undefined) process.env.REDIS_PORT = originalRedisPort;
    ...
    it('same request twice returns same response and avoids duplicate execution', async () => {
        const service = new OrderIdempotencyService({ ttlMs: 600000, lockTtlMs: 30000 });
        const execute = jest
            .fn()
