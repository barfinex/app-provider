...
export class OrderService {
  ...
  async getOpenOrders(options: {
    ...
    const data: Array<{ id: string; order: Order }> = [];
    let total = 0;
  ...
  async deleteAll(options: {
    ...
    const where = `connectorType='${connectorType}' AND marketType='${marketType}'`;
  ...
  private orderToOrderEntity(order: Order): OrderEntity {
    return {
      id:
        order.id ?? Date.now().toString() + Math.random().toString(16).slice(2),
