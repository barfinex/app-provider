...
export class OrderIdempotencyService implements OnModuleDestroy {
    ...
    private readonly lifecycleFile = 'apps/provider/src/order/order-idempotency.service.ts';
    private readonly lifecycleClientId = `provider-order-idempotency-${process.pid}-${Math.random().toString(36).slice(2, 10)}`;
    ...
    constructor(
        ...
        this.ttlMs = Number(process.env.PROVIDER_ORDER_IDEMPOTENCY_TTL_MS ?? options?.ttlMs ?? 600000);
    ...
    async tryStartProcessing(idempotencyKey: string): Promise<boolean> {
        ...
        if (redis) {
            try {
                const result = await redis.set(this.getProcessingKey(key), '1', 'PX', this.lockTtlMs, 'NX');
                return result === 'OK';
            } catch {
    ...
    async setFinalResponse<T>(idempotencyKey: string, response: T): Promise<void> {
        const key = this.normalizeKey(idempotencyKey);
        if (!key) return;
        ...
        if (redis) {
            try {
                await redis.set(this.getResultKey(key), JSON.stringify(response), 'PX', this.ttlMs);
                return;
            } catch {
                // Fall through to in-memory mode if Redis becomes unavailable during runtime.
            }
    ...
    private async getRedis(): Promise<Redis | null> {
        ...
        try {
            ...
            client.on('reconnecting', () => this.logRedisLifecycle('reconnect_attempt'));
            ...
            this.logger.warn(
                `[provider-idempotency] redis_unavailable reason=${String(error?.message ?? error ?? 'unknown')}`,
            );
            return null;
        }
    }
    ...
    private logRedisLifecycle(action: string, channels?: string | string[]): void {
