...
export class BinanceService implements OnModuleInit, OnModuleDestroy {
  ...
  getSymbolsWithMarketQualitySnapshots(): string[] {
  ...
  changeLeverage(symbol: TradingSymbol, leverage: number) {
  ...
  ): { validSymbols: TradingSymbol[]; removedSymbols: string[] } {
  ...
  async unsubscribe(): Promise<void> {
    ...
    if (this.subscription.unsubscribeSymbolPrices)
      this.subscription.unsubscribeSymbolPrices();
    if (this.subscription.unsubscribeSymbols)
  ...
  private tearDownMarketSub(
    ...
    if (sub.unsubscribeSymbolPrices) sub.unsubscribeSymbolPrices();
    if (sub.unsubscribeSymbols) sub.unsubscribeSymbols();
  ...
  async subscribe(
    ...
    if (estimatedStreams >= this.streamCardinalityWarnThreshold) {
      this.logger.warn(
        `[BINANCE WS] high stream cardinality estimate streams=${estimatedStreams} symbols=${
          symbols.length
        } intervals=${Math.max(
          1,
          intervals?.length ?? 0,
      ...
      try {
        switch (sub.type) {
          ...
          case SubscriptionType.PROVIDER_SYMBOLS: {
            ...
            const unsub = await this.ws.subscribeToSymbols(
              { marketType },
              symbolsHandler,
            );
            this.subscription.unsubscribeSymbols = unsub;
            marketSub.unsubscribeSymbols = unsub;
            break;
          }

          case SubscriptionType.PROVIDER_SYMBOL_PRICES: {
            const unsub = await this.ws.subscribeToSymbolPrices(
              {
                marketType,
            ...
            this.subscription.unsubscribeSymbolPrices = unsub;
            marketSub.unsubscribeSymbolPrices = unsub;
            break;
          }
        }
      } catch (err) {
  ...
  private async handlerForTrade(
    ...
    if (
      !this.symbolShardService.isSymbolInLocalShard(
        symbol,
        this.connectorType,
        marketType,
      )
    ...
    const scope = this.symbolShardService.getScopeForSymbol(
      symbol,
      this.connectorType,
      marketType,
    );
    ...
    const symbolStr = trade.symbol?.name ?? '';
      ...
      if (
        ...
        const symbolHint =
          type === SubscriptionType.PROVIDER_SYMBOLS
            ...
            : (value as any)?.symbol?.name ?? (value as any)?.symbol ?? '—';
  ...
  private evaluateAndTrackMarketQuality(
    ...
    if (cached && now - lastEval < BinanceService.QUALITY_EVAL_THROTTLE_MS) {
      return cached;
    }
  ...
  private logMarketQualitySummary(): void {
    const activeFromSubscriptions =
      this.subscription.options?.symbols?.map((symbol) => symbol?.name) ?? [];
