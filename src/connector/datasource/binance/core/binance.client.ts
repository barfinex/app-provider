...
export class BinanceClientService {
  ...
  private async init(): Promise<void> {
    ...
    try {
      ...
      const connectorSecret =
        connectorConfig.secret ?? process.env.BINANCE_API_SECRET;
      ...
      const fetchBinanceTime = async (): Promise<number> => {
        ...
        if (!res.ok) {
          this.logger.warn(
            `Binance time request failed: ${res.status} ${res.statusText}`,
          );
          throw new InternalServerErrorException(
            `Binance time request failed: ${res.status}. Body: ${raw.slice(
              0,
              200,
            )}`,
          );
        ...
        let json: { serverTime?: number; code?: number; msg?: string };
        try {
          json = JSON.parse(raw) as typeof json;
        } catch {
          this.logger.warn('Binance time response was not valid JSON');
          throw new InternalServerErrorException(
            `Binance time response not JSON. Body: ${raw.slice(0, 200)}`,
          );
        }
        ...
        // Binance error payload or unexpected shape
        const detail =
          json.code != null && json.msg != null
            ? `code=${json.code} msg=${json.msg}`
            : raw.slice(0, 200);
        this.logger.warn(
          `Invalid Binance time response (no serverTime): ${detail}`,
        );
        throw new InternalServerErrorException(
          `Invalid Binance time response: ${detail}`,
        );
      };
      ...
      try {
        ...
        this.logger.log(
          `🕒 Binance time: ${moment.utc(time).format('YYYY-MM-DD HH:mm:ss')}`,
        );
      } catch {
  ...
  async ensureReady(timeoutMs = 10_000): Promise<void> {
    ...
    await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error('BinanceClientService ensureReady timeout')),
          timeoutMs,
        ),
      ),
    ]);
  ...
  private async loadExchangeSymbols(
    kind: 'spot' | 'futures',
  ): Promise<string[]> {
    try {
      const response =
