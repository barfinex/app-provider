...
} from '../../../../connector/utils/trading-symbol-sanitizer';
...
export class BinanceWsManager {
    ...
    private readonly streamStallThresholdMsByType: Record<StreamType, number> = {
        candles: Math.max(
            1000,
            Number(process.env.BINANCE_WS_STALL_THRESHOLD_CANDLES_MS || this.streamStallThresholdDefaultMs),
        ),
        orderbook: Math.max(
            1000,
            Number(process.env.BINANCE_WS_STALL_THRESHOLD_ORDERBOOK_MS || this.streamStallThresholdDefaultMs),
        ),
        trades: Math.max(
            1000,
            Number(process.env.BINANCE_WS_STALL_THRESHOLD_TRADES_MS || this.streamStallThresholdDefaultMs),
        ),
        symbolPrices: Math.max(
            1000,
            Number(process.env.BINANCE_WS_STALL_THRESHOLD_SYMBOLPRICES_MS || this.streamStallThresholdDefaultMs),
        ),
    };
    ...
    private readonly lastAccountPayloadLogAtByMarket = new Map<MarketType, number>();
    ...
    getStreamHealth() {
        const streams = Array.from(this.streamStatus.entries()).map(([streamId, state]) => ({
            streamId,
            ...state,
    ...
    private getStallThresholdMs(streamType: StreamType): number {
        return this.streamStallThresholdMsByType[streamType] || this.streamStallThresholdDefaultMs;
    }
    ...
    private registerStream(streamId: string, streamType: StreamType, marketType: MarketType): void {
    ...
    private touchStream(streamId: string, streamType: StreamType, marketType: MarketType): void {
    ...
    private logWsEventSummaryIfDue(force = false): void {
        ...
        this.wsEventsByStream = { candles: 0, trades: 0, orderbook: 0, symbolPrices: 0 };
        if (candles + trades + orderbook + symbolPrices === 0) return;
        this.logger.log(
            `[BINANCE WS] aggregate candles_events_per_sec=${(candles / elapsedSec).toFixed(1)} `
            + `trades_events_per_sec=${(trades / elapsedSec).toFixed(1)} `
            + `orderbook_events_per_sec=${(orderbook / elapsedSec).toFixed(1)} `
            + `symbol_prices_events_per_sec=${(symbolPrices / elapsedSec).toFixed(1)} `
            + `window_ms=${elapsedMs}`,
        );
    }
    ...
    private logTopicDiagnostics(
        ...
        if (this.wsDebugEnabled) {
            this.logger.debug(
                `[BINANCE WS] market=${marketType} symbols=${symbolSourceList.join(',')} topics=${topics.join(',')}`,
            );
        }
    ...
    private logRemovedInvalidSymbols(rawSymbols: string[], validSymbols: string[]): void {
    ...
    private warnAccountPayloadSize(
        ...
        this.logger.warn(
            `[PayloadSizeWarning] channel=${String(channel)} size_bytes=${sizeBytes} positions=${positionsCount} assets=${assetsCount}`,
        );
    }
    ...
    private compactFuturesAccountEventPayload(data: any): {
        ...
        const topLevelPositions = Array.isArray(data?.positions) ? data.positions : [];
        ...
        const rawPositions = Math.max(topLevelPositions.length, nestedPositions.length);
        const rawAssets = Math.max(topLevelAssets.length, nestedAssets.length);

        const activeTopLevelPositions = this.filterActiveFuturesPositions(topLevelPositions);
        const activeNestedPositions = this.filterActiveFuturesPositions(nestedPositions);
        ...
        const activeAssets = Math.max(activeTopLevelAssets.length, activeNestedAssets.length);

        const payload = {
            ...data,
            positions: Array.isArray(data?.positions) ? activeTopLevelPositions : data?.positions,
            balances: Array.isArray(data?.balances) ? activeTopLevelAssets : data?.balances,
            a: data?.a
                ? {
                    ...data.a,
    ...
    private sanitizeSymbolObjects(marketType: MarketType, symbols: Symbol[]): Symbol[] {
        const rawNames = symbols.map((s) => s?.name ?? '');
        const { validSymbols, removedSymbols } = this.clientService.validateBinanceSymbols(
            marketType,
            rawNames,
        );
        ...
        return symbols.filter((s) => {
            ...
            if (!normalized || !validSet.has(normalized) || dedup.has(normalized)) return false;
            dedup.add(normalized);
            return true;
        });
    }

    private sanitizeSymbolNames(marketType: MarketType, symbols: string[]): string[] {
        const { validSymbols, removedSymbols } = this.clientService.validateBinanceSymbols(
            marketType,
            symbols,
        );
    ...
    async subscribeToCandles(
        ...
        const method =
            marketType === MarketType.futures
                ? 'futuresCandles'
                : 'candles';
        const topics = sanitizedSymbols.map((s) => `${s.name.toLowerCase()}@kline_${convertTimeFrame(interval)}`);
        ...
        const wrappedHandler = (data: any) => {
            this.recordWsEvent('candles');
            this.touchStream(streamId, 'candles', marketType);
            reconnectAttempt = 0;
        ...
        const watchdog = setInterval(() => {
            ...
            reconnectAttempt += 1;
            const reconnectDelayMs = this.getReconnectDelayWithBackoffMs(reconnectAttempt);
            if (now - lastStallWarnAt >= this.stallWarnThrottleMs) {
                this.logger.warn(
                    `[BINANCE WS] stall detected stream=${streamId} thresholdMs=${stallThresholdMs} ageMs=${now - lastMessageAt}; reconnecting attempt=${reconnectAttempt} delayMs=${reconnectDelayMs}`,
                );
                lastStallWarnAt = now;
            }
    ...
    async subscribeToOrderBook(
        ...
        const wrappedHandler = (data: any) => {
            this.recordWsEvent('orderbook');
            this.touchStream(streamId, 'orderbook', marketType);
            reconnectAttempt = 0;
            stallCooldownUntil = 0;
            this.marketdataMetrics.observeMarketDataLag(
                'orderbook',
    ...
    async subscribeToTrade(
        ...
        const topics = sanitizedSymbols.map((s) => `${s.name.toLowerCase()}@aggTrade`);
        this.logTopicDiagnostics(marketType, sanitizedSymbols, topics);
        if (this.wsDebugEnabled) {
            this.logger.debug(
        ...
        const wrappedHandler = (data: any) => {
            this.recordWsEvent('trades');
            this.touchStream(streamId, 'trades', marketType);
            reconnectAttempt = 0;
            stallCooldownUntil = 0;
            this.marketdataMetrics.observeMarketDataLag(
                'trades',
    ...
    async subscribeToAccount(
        ...
        const wrappedHandler = (data: any) => {
            ...
            let positionsCount = Array.isArray(data?.positions) ? data.positions.length : 0;
            let assetsCount = Array.isArray(data?.balances) ? data.balances.length : 0;
        ...
        const unsubscribe = await Promise.resolve(this.api.ws[method](wrappedHandler));
    ...
    async subscribeToSymbolPrices(
        ...
        if (marketType === MarketType.futures) {
            this.webSocketService.onOpen(wsUrl, (ws) => {
                const params = sanitizedSymbols.map(
