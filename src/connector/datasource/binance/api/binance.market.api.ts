...
import { ConnectorType, MarketType, TradingSymbol } from '@barfinex/types';
        ...
        if (marketType === MarketType.spot) {
          ...
          exchangeInfo.symbols.forEach((item) => {
            result.push({
              ...
              status: item.status,
              minPrice: item.filters.find(
                (filter) => filter.filterType === 'PRICE_FILTER',
              )?.minPrice,
              maxPrice: item.filters.find(
                (filter) => filter.filterType === 'PRICE_FILTER',
              )?.maxPrice,
              minQuantity: item.filters.find(
                (filter) => filter.filterType === 'LOT_SIZE',
              )?.minQty,
              stepSize: item.filters.find(
                (filter) => filter.filterType === 'LOT_SIZE',
              )?.stepSize,
              tickSize: item.filters.find(
                (filter) => filter.filterType === 'PRICE_FILTER',
              )?.tickSize,
          ...
          const securityConfig = connectors.find(
            (q: { connectorType: any }) =>
              q.connectorType === ConnectorType.binance,
          );
