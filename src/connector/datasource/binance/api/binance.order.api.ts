...
export class BinanceOrderApi {
        ...
        if (!order.useSandbox) {
            switch (order.marketType) {
                ...
                case MarketType.futures: {
                    ...
                    const existing = await this.getOpenOrders({ marketType: order.marketType });
    ...
    async closeOrder(options: {
        ...
        if (!provider) throw new InternalServerErrorException('Provider config is missing');
    ...
    async getOpenOrders(options: {
        ...
        if (marketType === MarketType.spot) {
            const orders = await api.openOrders(symbol ? { symbol: symbol.name } : {});
            orders.forEach(o => {
                result.push({
            ...
            const orders = await api.futuresOpenOrders(symbol ? { symbol: symbol.name } : {});
            orders.forEach(o => {
