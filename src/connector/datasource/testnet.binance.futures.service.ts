...
export class TestnetBinanceFuturesService implements OnModuleInit, DataSource {
  ...
  private subscription: {
    options?: { symbols: TradingSymbol[]; intervals?: TimeFrame[] };
  ...
  private lastAccountAdapterEventTime = 0;
  ...
  private readonly badStatus = [
    'CANCELED',
    'EXPIRED',
    'PENDING_CANCEL',
    'REJECTED',
  ];
  private readonly ignoredErrorsList = [
  ...
  async getPrices(
    ...
    if (!this.api) {
      this.logger.warn(
        '[BinanceService] API not initialized, returning empty prices',
      );
      return result;
    }
    ...
    symbols.forEach((symbol) => {
      if (exchangePrices[symbol.name])
        result[symbol.name] = {
          value: Number(exchangePrices[symbol.name]),
          moment: exchangeTime,
        };
    });
  ...
  async getAssetsInfo(
    ...
    switch (marketType) {
      case MarketType.spot:
        ...
        const accountInfoSpot = (await this.api?.accountInfo()).balances.filter(
      ...
      case MarketType.futures:
        ...
        const accountInfoFutures_Assets = accountInfoFutures.assets.filter(
          (q) =>
            parseFloat(q.walletBalance) != 0 || parseFloat(q.availableBalance),
        );
        ...
        const accountInfoFutures_Positions =
          accountInfoFutures.positions.filter(
  ...
  async getAccountInfo(marketType: MarketType): Promise<Account> {
    ...
    switch (marketType) {
      case MarketType.futures:
        ...
        const filterAssets = account.assets.filter(
          (q: any) => q.symbol.name != currency,
        );
        ...
        for (let i = 0; i < filterAssets.length; i++) {
          ...
          const orders = await this.api
            ?.futuresOpenOrders({ symbol: asset.symbol + currency } as any)
            .catch((e) => {
          ...
          if (!isSymbolIgnore)
            account.symbols.push({ name: asset.symbol + currency });
  ...
  async changeLeverage(symbol: TradingSymbol, newLeverage: number): Promise<TradingSymbol> {
    if (!this.api) {
      this.logger.warn(
        '[BinanceService] API not initialized, returning empty prices',
      );
    ...
    const result = await this.api?.futuresLeverage({
      symbol: symbol.name,
        ...
        case MarketType.futures:
          ...
          if (
            !orders.find(
              (q) =>
                q.symbol &&
                q.symbol.name == futuresOrderToProvider.symbol &&
                q.type == futuresOrderToProvider.type &&
                q.side == futuresOrderToProvider.side,
            )
          )
            futuresOrderEntity = await this.api?.futuresOrder(
              futuresOrderToProvider,
            );
  ...
  async closeOrder(options: {
    ...
    if (element)
      await this.api?.futuresCancelOrder({ orderId: +id, symbol: symbol.name });
  ...
  public async getOpenOrders(options: {
    ...
    if (marketType == MarketType.spot) {
      const ordersInProvider_Spot = await this.api?.openOrders(
      ...
      const ordersInProvider_Futures = await this.api?.futuresOpenOrders(
        symbol ? { symbol: symbol.name } : {},
      );
  ...
  public async subscribe(
    ...
    this.subscription.unsubscribeAccount = await this.subscribeToAccount(
      { marketType },
  ...
  //     //GlobalService.providers[this.options.connectorType].subscribedAssets[symbol] = { symbol: TradingSymbol }
  ...
  private handlerForAccount = async (
    marketType: MarketType,
  ...
  private getMatchedSubscription(
    ...
    const matchedSubscription = this.configService
      ...
      ?.subscriptions?.find((s: any) => s.type === subscriptionType);
  ...
  private handlerForOrderbook = async (
    marketType: MarketType,
    orderbook: OrderBook,
  ) => {
  ...
  private handlerForTrade = async (marketType: MarketType, trade: Trade) => {
    ...
    const subscription: Subscription = {
      type: subscriptionType,
      updateMoment: Date.now(),
      symbols: this.getMatchedSubscription(
        this.connectorType,
        marketType,
        subscriptionType,
      ).symbols,
  ...
  public async subscribeToСandles(
    ...
    if (!this.api || !this.api.ws) {
      this.logger.warn(
        `[BinanceService] API not initialized, cannot subscribeToOrderBook`,
      );
      return () => {};
    }
  ...
  public async subscribeToOrderBook(
    ...
    const unsubscribe = this.api?.ws[method](
      symbols.map((s) => s.name),
      this.orderBookAdapter(marketType, handler),
    );
  ...
  public async subscribeToAccount(
    ...
    this.api?.ws.trades(['BTCUSDT'], (trade) =>
      console.log('Number(trade.price):', Number(trade.price)),
    );

    const unsubscribe = await this.api?.ws[method](
      this.accountAdapter(marketType, handler),
    );
  ...
  public async subscribeToTrade(
    ...
    const unsubscribe = this.api?.ws[method](
      symbols.map((s) => s.name),
      this.tradeAdapter(marketType, handler),
    );
  ...
  private orderBookAdapter(marketType: MarketType, handler: OrderBookHandler) {
    ...
    return (msg: BinanceDepth) => {
      ...
      handler(marketType, { bids, asks, symbol, time });
    };
  }
