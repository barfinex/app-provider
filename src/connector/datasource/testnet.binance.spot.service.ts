    ...
    switch (marketType) {
            ...
            price: [
              {
                ...
                value:
                  element.asset == currency
                    ? parseFloat('1')
                    : parseFloat(
                        pricesSpot[element.asset + currency]
                          ? pricesSpot[element.asset + currency]
                          : '0',
                      ),
              },
      ...
      case MarketType.futures:
        ...
        for (let i = 0; i < filterAssets.length; i++) {
          ...
          const orders = await this.api.futuresOpenOrders({
            symbol: asset.symbol + currency,
          } as any);
  ...
  private getMatchedSubscription(
    ...
    const matchedConnector = config.provider.connectors.find(
      (c: any) =>
        ...
        c.markets?.some(
          (m: { marketType: MarketType }) => m.marketType === marketType,
        ),
    );
