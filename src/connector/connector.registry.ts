...
export class ConnectorRegistry {
  ...
  static getConnector(options: {
    ...
    if (!options.marketType) {
      console.warn(
        `No marketType provided for connector ${options.connectorType}`,
      );
      return undefined;
    }
  ...
  static addSubscription(options: {
    ...
    if (!connector) {
      console.warn(
        `[ConnectorRegistry] addSubscription skipped — connector not found: ${key}`,
      );
      return;
    }
    ...
    const subscriptionIndex = connector.subscriptions.findIndex(
    ...
    if (subscriptionIndex === -1) {
      ...
      connector.subscriptions[subscriptionIndex] = options.subscription;
    }
  }
