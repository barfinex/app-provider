import { TradingSymbol } from '@barfinex/types';
...
export function normalizeTradingSymbol(
...
export function isValidTradingSymbol(
  symbol: string | null | undefined,
): boolean {
...
export function sanitizeTradingSymbols(
  symbols: string[],
): SymbolSanitizerResult {
...
export function sanitizeTradingSymbolObjects(symbols: TradingSymbol[]): {
  ...
  return {
    validSymbols: symbols.filter((s) =>
      validSet.has(normalizeTradingSymbol(s?.name)),
    ),
    removedSymbols,
  };
}
