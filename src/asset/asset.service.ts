...
export class AssetService {
  ...
  constructor(
    private readonly accountService: AccountService,
    private readonly connectorService: ConnectorService, // @InjectModel(OrderEntity.name) private readonly orderEntity: Model<OrderEntityDocument>,
  ) {}
  ...
  async getAll(): Promise<
    { connectorType: string; marketType: string; details: any }[]
  > {
    const result = [];
    ...
    for (let i = 0; i < connectorTypes.length; i++) {
      ...
      for (let j = 0; j < marketTypes.length; j++) {
        ...
        const details = await this.getAllAsset(
          ConnectorType[connectorType as keyof typeof ConnectorType],
          MarketType[marketType as keyof typeof MarketType],
        );
